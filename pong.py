...
batch_size = 10 # number of episodes before updating our network parameters
learning_rate = 1e-4
...
def policy_forward(x):
  ...
  logp = np.dot(model['W2'], h) # logp is a scalar
  p = sigmoid(logp)
...
def policy_backward(eph, epdlogp):
  ...
  """
  dW2 = np.dot(eph.T, epdlogp).ravel() # derivs wrt W2
  dh = np.outer(epdlogp, model['W2']) # derivs wrt h post activation
  dh[eph <= 0] = 0 # backprop relu
  dW1 = np.dot(dh.T, epx) # derivs wrt W1
...
while True:
  ...
  if done: # an episode finished, accumulate gradients or backprop
    ...
    epx = np.vstack(xs)
    eph = np.vstack(hs)
    epdlogp = np.vstack(dlogps) # episode gradients of y - aprob
    epr = np.vstack(drs)
